...
| https://www.amazon.com/gp/help/customer/display.html?ref_=hp_left_v4_sib&nodeId=GQT5HV6YYGNDSFNW | DELETE | Delete candidate intent `manage_wish_list_address`. | The page does not natively handle wish list addresses; it redirects to "Create Your List". | "For wish lists, visit Create Your List." | Yes |
...
|---|---|---|
| Proposal 1 (MERGE): Merge address management actions into `manage_addresses` | Approved | Consolidating actions performed in a single settings screen simplifies taxonomy. |
| Proposal 2 (DELETE): Delete `change_order_address` | Approved | Redirects to external page. |
| Proposal 3 (DELETE): Delete `manage_wish_list_address` | Approved | Redirects to external page. |

### Final Curation Decisions (Iteration 1)
Applied all approved proposals:
- Merged `add_address`, `edit_address`, `delete_address`, and `set_default_address` into `manage_addresses`.
- Deleted `change_order_address` and `manage_wish_list_address` as they point to external help pages.
