...
print("Pre-computing Intent Description Embeddings for Two-Tower Validation...")
...
def get_hybrid_intents(question, query_embedding_raw):
    ...
    # 1. Multi-Stage Thresholds
    if top1_prob < 0.13:
        is_llm_fallback = True
    elif margin < 0.15:
        is_llm_fallback = True
        
    # 2. Two-Tower Validation (Disagreement check)
    if not is_llm_fallback and top1_prob < 0.85:
...
print("Running E-to-E Evaluation (ret_v3 architecture)...")
total = len(qa_df)
...
for idx, row in qa_df.iterrows():
    ...
    predicted_intents, is_fallback, top1_prob, margin = get_hybrid_intents(q, q_emb_raw)
    ...
    if not filtered_indices:
        for k in k_values: mrr_scores[k].append(0)
        failed_queries_k3.append({"question": q, "reason": "Filtered DF empty"})
        continue
    ...
    for local_idx in top_k_local:
        ...
        bi_scores.append(float(similarities[local_idx]))
        
    cross_scores = reranker.predict(pairs)
    
    final_ranked_indices = np.argsort(cross_scores)[::-1]
    final_ranked_chunk_ids = [chunk_ids[i] for i in final_ranked_indices]
    
    for k in k_values:
        top_k_cids = final_ranked_chunk_ids[:k]
        if expected_chunk in top_k_cids:
            hit_rates[k] += 1
            rank = top_k_cids.index(expected_chunk) + 1
            mrr_scores[k].append(1.0 / rank)
        else:
            mrr_scores[k].append(0)
            if k == 3:
                top_3_retrieved = []
                for j in range(min(3, len(final_ranked_indices))):
                    original_idx = final_ranked_indices[j]
                    top_3_retrieved.append({
                        "chunk_id": chunk_ids[original_idx],
                        "cross_score": float(cross_scores[original_idx]),
                        "bi_score": bi_scores[original_idx]
                    })
                
                failed_queries_k3.append({
                    "question": q,
                    "expected_chunk": expected_chunk,
                    "expected_intent": expected_intent,
                    "predicted_intents": predicted_intents,
                    "is_fallback": is_fallback,
                    "top_retrieved": top_3_retrieved
                })
...
with open(os.path.join(ws_dir, "scratch", "eval_v6_failures.json"), "w") as f:
    json.dump({"3": {"failed_cases": failed_queries_k3}}, f, indent=2)
    
print("Saved failures to eval_v6_failures.json")
