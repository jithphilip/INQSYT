...
with open(os.path.join(ws_dir, "scratch", "eval_v2_failures.json"), "r") as f:
    data = json.load(f)
...
md = []
md.append("# 📊 RAG Evaluation & Failure Analysis Report (Iteration 02: 10k Natural LR)\n")
...
md.append("| :--- | :---: | :---: | :--- | :--- |")
md.append(f"| NLU Classifier Misclassification | NLU Classifier/Logistic Regression | {len(nlu_failures)}/{(len(nlu_failures)/len(failed_cases))*100:.1f}% | The LR model routed the query to the wrong intent, completely missing the true chunk's domain before vector search even began. | Train the BERT classifier (Iteration 04) for much higher semantic understanding of complex intents. |")
md.append(f"| Reranking / Ranking Failure | Reranker/Cross-Encoder | {len(reranker_failures)}/{(len(reranker_failures)/len(failed_cases))*100:.1f}% | The intent was predicted correctly, but the exact target chunk was out-scored by other chunks within the same intent domain by the Cross-Encoder. | Implement dynamic chunking or refine Cross-Encoder fine-tuning for domain-specific jargon. |")
md.append(f"| LLM Router Fallback Error | LLM Router/Few-Shot Fallback | 0/0.0% | The LLM fallback either wasn't triggered or correctly aligned with the NLU failure. | Adjust margin thresholds for LLM routing. |\n")
...
out_path = os.path.join(artifacts_dir, "iteration_02_hybrid_retriever_LR.md")
