...
BERT_DIR = os.path.join(GEN_DIR, "bert_classifier")

# 1. Load resources
print("Loading data...")
qa_df = pd.read_csv(os.path.join(ws_dir, "Main_Data", "eval_dataset.csv"))
chunks_df = pd.read_json(os.path.join(GEN_DIR, "Chunks.jsonl"), lines=True)

with open(os.path.join(GEN_DIR, "intents_schema.json"), "r", encoding="utf-8") as f:
...
index = faiss.read_index(os.path.join(GEN_DIR, "faiss_index.bin"))
...
classes = bert_meta["classes"]

def get_hybrid_intents_bert(question):
    # 1. Run BERT NLU
    inputs = tokenizer(question, return_tensors="pt", truncation=True, max_length=64).to(device)
    with torch.no_grad():
        outputs = bert_model(**inputs)
        probs_tensor = F.softmax(outputs.logits, dim=1)[0]
...
for k in k_values:
    ...
    for idx in range(len(qa_df)):
        ...
        # 1. Routing
        predicted_intents, is_fallback, top1_prob, margin = get_hybrid_intents_bert(question)
...
# Print Summary Table
print("\n" + "="*50)
print("BERT HYBRID RETRIEVER EVALUATION RESULTS")
print("="*50)
...
with open(os.path.join(ws_dir, "scratch", "eval_bert_failures.json"), "w") as f:
    json.dump(evaluation_report, f, indent=2)
print(f"Saved failure details to scratch/eval_bert_failures.json")
