...
HF_REPO_ID = "anupam-dagupta-cmi/INQSYT"
HF_PATH_IN_REPO = "bert_classifier"
...
def upload_model_to_hf():
    """
    Uploads the locally saved BERT classifier folder to Hugging Face.

    Requires either:
    1. You already ran: hf auth login
    OR
    2. You set an HF_TOKEN environment variable.
    """

    print(f"\nUploading model folder to Hugging Face repo: {HF_REPO_ID}/{HF_PATH_IN_REPO}")
    ...
    api.upload_folder(
        ...
        commit_message="Upload updated BERT intent classifier"
    )

    print("Upload to Hugging Face completed successfully.")


def train_bert(upload_to_hf=True):
    # 1. Load dataset
    ...
    print("Done! BERT classifier trained and saved locally.")
