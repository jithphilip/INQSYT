# 📊 RAG Evaluation & Failure Analysis Report (Iteration 4: Linearized Markdown)
...
| :--- | :---: | :---: | :--- | :--- |
| **NLU Classifier Misclassification** | NLU Classifier/Logistic Regression | 16 / 51.6% | The logistic regression model failed to include the true intent in the top 3 predicted intents. | Add more varied training data for these specific intents. |
...
| **Reranking / Ranking Failure** | Reranker/Cross-Encoder | 15 / 48.4% | The router correctly identified the intent, but the Cross-Encoder scored irrelevant chunks higher than the true chunk. | Refine chunks further or fine-tune the Cross-Encoder. |
...
*(Comparing Iteration 4 against Iteration 2 (Baseline Markdown))*

* **What Improved**: The Hit Rate @ 3 rebounded perfectly back to **90.88%** from the 89.41% drop in Iteration 3. The Bi-Encoder successfully used the flattened markdown data to recapture all dropped queries.
* **What Regressed**: No regressions noted. The NLU logic remained perfectly stable.
* **Correlation**: By linearizing the structural noise into space-separated plaintext for the FAISS index, we starved the Bi-Encoder of syntax noise while feeding it maximum semantic density. However, because we ran it on `chunks_v1.jsonl` (which contains useless boilerplate text instead of the subagent's clean semantic summaries), we only matched the 90.88% baseline ceiling rather than pushing past it.